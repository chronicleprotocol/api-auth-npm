...
import { AuthTokenError, AuthTokenErrorCode, deriveKeyFromCredentials, } from "./credentials.js";
...
export async function signAuthTokenFromCredentials({ username, password, duration, }) {
    if (typeof username !== "string" ||
        typeof password !== "string" ||
        !username ||
        !password) {
        throw new AuthTokenError(AuthTokenErrorCode.MISSING_FIELDS, "Username and password are required");
    }
    ...
    if (typeof duration !== "undefined") {
        if (typeof duration !== "number" ||
            ...
            duration <= 0) {
            throw new AuthTokenError(AuthTokenErrorCode.INVALID_DURATION, "Duration, if provided, must be a positive integer number of seconds");
        }
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : "Failed to generate auth token";
